...
var request_female = {
    ...
    "variables": [
        ...
        //"employment_female_education_legal_community_service_arts_and_media_occupations",//4
        ...
        //"employment_female_management_business_and_financial_occupations", //5
        //"employment_female_management_business_science_and_arts_occupations",//1
        "employment_female_management_occupations",
        ...
        //"employment_female_office_and_administrative_support_occupations",//2
        ...
        //"employment_female_sales_and_office_occupations",//3
...
var loadData = false;
function preload() {
  // load geoJason states data (from http://eric.clst.org/Stuff/USGeoJSON)
  loadJSON("us_states_geo.json", function(response){
  ...
  census.APIRequest(request_female, function(response) {
    ...
    responseData.female[state].maxVal = 0;
    responseData.female[state].maxKey = "";
    var total = 0;
    for (key in dataObject[i]) {
      if (key!="name" && key!="state") {
        ...
        if (responseData.female[state][key]>responseData.female[state].maxVal) {
          responseData.female[state].maxVal = responseData.female[state][key]
          responseData.female[state].maxKey = key
        }
      }
    }
...
function addDataToGeoJson() {
  for (var i=0; i<geoJson.length; i++) {
    geoJson[i].properties.maxOccupation = responseData.female[geoJson[i].properties.NAME].maxKey
  }
  loadData=false;
  loadGeo = false;
...
function draw() {
  ...
  for (state in responseData.female) {
    ...
    for (key in responseData.female[state]) {
      if (key != "total" && key!="maxVal" && key!="maxKey") {
        rotate(angle);
